...
export function PopupWidget() {
  ...
  const onSubmit = async (data: any, e: any) => {
    ...
    await fetch("https://api.web3forms.com/submit", {
      ...
      .catch((error) => {
        setIsSuccess(false);
        setMessage("Error del cliente. Revisa la consola para más información");
        console.log(error);
      });
  };
  ...
  return (
    <div>
      <Disclosure>
        {({ open }) => (
          <>
            <DisclosureButton className="fixed z-40 flex items-center justify-center transition duration-300 bg-[#1A4670] rounded-full shadow-lg right-5 bottom-5 w-14 h-14 focus:outline-none hover:bg-[#153755] focus:bg-[#153755] ease">
            ...
            >
              <DisclosurePanel className="flex flex-col overflow-hidden left-0 h-full w-full sm:w-[350px] min-h-[250px] sm:h-[600px] border border-gray-300 dark:border-gray-800 bg-white shadow-2xl rounded-md sm:max-h-[calc(100vh-120px)]">
                <div className="flex flex-col items-center justify-center h-32 p-5 bg-[#1A4670]">
                  <h3 className="text-lg text-white">¿Cómo podemos ayudarte?</h3>
                  <p className="text-white opacity-50">Normalmente respondemos en unas pocas horas</p>
                </div>
                ...
                <div className="flex-grow h-full p-6 overflow-auto bg-gray-50">
                    ...
                    <form onSubmit={handleSubmit(onSubmit)} noValidate>
                      ...
                      <div className="mb-4">
                        ...
                        <input
                          ...
                          {...register("name", { required: "El nombre completo es obligatorio", maxLength: 80 })}
                          className={`w-full px-3 py-2 text-gray-600 placeholder-gray-300 bg-white border border-gray-300 rounded-md focus:outline-none focus:ring ${
                            errors.name
                              ? "border-red-600 focus:border-red-600 ring-red-100"
                              : "border-gray-300 focus:border-[#E0F7FA] ring-[#E0F7FA]"
                          }`}
                        />
                        {errors.name && <div className="mt-1 text-sm text-red-400 invalid-feedback">{errors.name.message as string}</div>}
                      </div>
                      ...
                      <div className="mb-4">
                        ...
                        <input
                          ...
                          {...register("email", {
                            ...
                            pattern: { value: /^\S+@\S+$/i, message: "Por favor ingresa un correo válido" },
                          })}
                        ...
                        />
                        {errors.email && <div className="mt-1 text-sm text-red-400 invalid-feedback">{errors.email.message as string}</div>}
                      </div>
                      ...
                      <div className="mb-4">
                        ...
                        <textarea
                          ...
                          required
                        ></textarea>
                        {errors.message && <div className="mt-1 text-sm text-red-400 invalid-feedback">{errors.message.message as string}</div>}
                      </div>

                      <div className="mb-3">
                        <button
                          type="submit"
                          className="w-full px-3 py-4 text-white bg-[#1A4670] rounded-md focus:bg-[#153755] focus:outline-none"
                        >
                          {isSubmitting ? (
                  ...
                  {isSubmitSuccessful && isSuccess && (
                    <div className="flex flex-col items-center justify-center h-full text-center text-white rounded-md">
                      ...
                      <h3 className="py-5 text-xl text-green-500">Mensaje enviado con éxito</h3>
                      <p className="text-gray-700 md:px-3">{Message}</p>
                      <button className="mt-6 text-[#1A4670] focus:outline-none" onClick={() => reset()}>
                        Volver
                      </button>
                    </div>
                  )}
